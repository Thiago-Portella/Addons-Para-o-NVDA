...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def script_save_note_to_memory(self, gesture):
        ...
        if textInfo is not None:
            ...
            if len(text) > 0:
                ...
                for part in parts:
                    part = part.strip()
                    if part:  # Apenas adiciona se houver conteúdo
                        self.memory.append(part)
                
                self.index = len(self.memory) - 1
                
                tones.beep(880, 100)  # Beep para confirmar.
                ui.message(f"{len(parts)} notes added. Current: {self.index+1} {self.memory[self.index]}")
            else:
                # Caso nenhum texto tenha sido selecionado
    ...
    @script(
        description="Paste all notes into the current application"
    )
    def script_paste_all_notes(self, gesture):
        if len(self.memory) > 0:
            combined_notes = "\n///\n".join(self.memory)

            api.copyToClip(combined_notes)
            keyboardHandler.KeyboardInputGesture.fromName("CONTROL+V").send()
            tones.beep(1080, 200)
            ui.message(f"Pasted {len(self.memory)} notes.")
        else:
            tones.beep(180, 220)
    ...
    __gestures={
        ...
        "kb:NVDA+CONTROL+SHIFT+A": "paste_all_notes"
    }